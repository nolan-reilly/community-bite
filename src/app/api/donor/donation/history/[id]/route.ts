...
// GET: api/donor/donation/history/[email_id]
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
    try {
        const email = (await params).id;
        const donationHistory = await prisma.$queryRaw
                                            `
                                            SELECT FB.business_name, FB.email as foodBank_email, FB.donation_status,
                                                   FB.street || ' ' || FB.city || ',' || FB.state || ' ' || FB.zipcode as pantry_address, 
                                                   DP.produce_name, DP.quantity, DP.weight, DP.proposed_date, DP.created_on,
                                                   DP.frequency, DP.comment, DP.post_id
                                            FROM (SELECT * 
